...
exports.planResearchStrategy = planResearchStrategy;
 ...
 */
async function planResearchStrategy(question) {
    const startTime = Date.now();
    v2_1.logger.info(`🧠 [LATENTS-PLANNER] Starting research planning for query: "${question.substring(0, 100)}..."`);
    try {
...
Return JSON with: estimatedRounds, strategy, focusAreas[]`
        ...
        try {
            ...
            v2_1.logger.error(`❌ [LATENTS-PLANNER] Failed to parse JSON response. Raw: ${text.substring(0, 200)}`);
            // Fallback plan
            plan = {
                estimatedRounds: 2,
        ...
        v2_1.logger.info(`✅ [LATENTS-PLANNER] Planning complete in ${duration}ms: ` +
        ...
        const duration = Date.now() - startTime;
        v2_1.logger.error(`❌ [LATENTS-PLANNER] Planning failed after ${duration}ms:`, error);
        // Return fallback plan
        v2_1.logger.warn(`⚠️ [LATENTS-PLANNER] Using fallback 2-round plan`);
        return {
            estimatedRounds: 2,
...
//# sourceMappingURL=latents-planner.js.map