...
exports.rankSourcesByRelevance = rankSourcesByRelevance;
exports.reorderSourcesByRanking = reorderSourcesByRanking;
 ...
 */
async function rankSourcesByRelevance(query, sources, config) {
    const startTime = Date.now();
    v2_1.logger.info(`🎯 [SOURCE-RANKER] Starting keyword-based ranking: ` +
    ...
    try {
        ...
        const allSources = [
            ...sources.pubmed.map(s => ({ source: s, sourceType: 'pubmed' })),
            ...sources.medrxiv.map(s => ({ source: s, sourceType: 'medrxiv' })),
            ...sources.clinicalTrials.map(s => ({ source: s, sourceType: 'clinicaltrials' })),
            ...sources.exa.map(s => ({ source: s, sourceType: 'exa' }))
        ];
        // Score each source
        const rankedSources = allSources.map((item) => {
        ...
        v2_1.logger.info(`✅ [SOURCE-RANKER] Ranking complete in ${duration}ms: ` +
        ...
        v2_1.logger.error(`❌ [SOURCE-RANKER] Ranking failed:`, error);
        throw error;
    }
}
/**
 ...
 */
function reorderSourcesByRanking(sources, rankingResult) {
    const pubmedReordered = [];
    const medrxivReordered = [];
    const trialsReordered = [];
    const exaReordered = [];
    for (const ranked of rankingResult.topSources) {
        if (ranked.sourceType === 'pubmed')
            pubmedReordered.push(ranked.source);
        else if (ranked.sourceType === 'medrxiv')
            medrxivReordered.push(ranked.source);
        else if (ranked.sourceType === 'clinicaltrials')
            trialsReordered.push(ranked.source);
        else if (ranked.sourceType === 'exa')
            exaReordered.push(ranked.source);
    }
    return {
...
function calculateKeywordScore(content, keywords) {
    if (keywords.length === 0)
        return 35; // Neutral score if no keywords
    let matches = 0;
 ...
 */
function extractTitle(source, sourceType) {
    switch (sourceType) {
        case 'pubmed':
 ...
 */
function extractAbstract(source, sourceType) {
    switch (sourceType) {
        case 'pubmed':
 ...
 */
function getCredibilityBoost(sourceType) {
    switch (sourceType) {
        case 'pubmed':
        case 'clinicaltrials':
 ...
 */
function extractPublishDate(source, sourceType) {
    switch (sourceType) {
        case 'pubmed':
 ...
 */
function getRecencyBoost(source, sourceType) {
    ...
    try {
        ...
        if (yearsDiff <= 1)
            return 15; // Very recent (within 1 year)
        if (yearsDiff <= 3)
            return 10; // Recent (within 3 years)
        if (yearsDiff <= 5)
...
//# sourceMappingURL=source-ranker.js.map