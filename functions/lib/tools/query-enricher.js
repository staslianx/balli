...
Enrich this query for medical search. Respond with JSON only.`;
    try {
        ...
        const result = await genkit_instance_1.ai.generate({
            model: (0, providers_1.getRouterModel)(), // Fast Flash model
            config: {
                temperature: 0.1,
                maxOutputTokens: 150
            },
        ...
        console.log(`✅ [QUERY-ENRICHER] "${input.currentQuestion}" → "${parsed.enriched}" (${duration}ms)\n` +
...
//# sourceMappingURL=query-enricher.js.map