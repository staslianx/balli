 ...
 */
export async function refineQueryForGaps(
  ...
  logger.info(
    `🔄 [QUERY-REFINER] Refining query for Round ${roundNumber}: ` +
    `gaps=[${gaps.slice(0, 3).join(', ')}${gaps.length > 3 ? '...' : ''}]`
  );

  // If no gaps, return original query
  if (gaps.length === 0) {
    logger.debug(`🔄 [QUERY-REFINER] No gaps identified, using original query`);
    return {
...
Return JSON with: refined, focusArea, reasoning`
    ...
    let refinement: Partial<RefinedQuery>;

    try {
      refinement = JSON.parse(jsonText);
    } catch (parseError) {
      logger.error(`❌ [QUERY-REFINER] Failed to parse JSON response. Raw: ${text.substring(0, 200)}`);
    ...
    logger.info(
      `✅ [QUERY-REFINER] Query refined in ${duration}ms: ` +
    ...
    const duration = Date.now() - startTime;
    logger.error(`❌ [QUERY-REFINER] Refinement failed after ${duration}ms:`, error);

    // Fallback: Combine original query with primary gap
    const primaryGap = gaps[0];
    logger.warn(`⚠️ [QUERY-REFINER] Using fallback gap append strategy`);
...
export async function generateQueryVariations(
  ...
  logger.debug(`🔄 [QUERY-REFINER] Generating ${count} query variations`);
...
Return JSON array of ${count} query strings.`
    ...
    try {
      ...
      if (Array.isArray(variations) && variations.length > 0) {
        logger.debug(`✅ [QUERY-REFINER] Generated ${variations.length} variations`);
        return variations.slice(0, count);
      }
    } catch (parseError) {
      logger.warn(`⚠️ [QUERY-REFINER] Failed to parse variations, using original`);
    }
  } catch (error: any) {
    logger.error(`❌ [QUERY-REFINER] Variation generation failed:`, error);
  }
