...
export interface SelectedSource {
  ...
  summary: string; // Abstract or snippet
  credibilityBadge: string; // 'highly_credible', 'credible', 'standard'
  estimatedTokens: number; // Estimated token count for this source
}
...
export interface SourceSelectionResult {
  ...
  totalSources: number; // Original count
  selectedCount: number; // Final count after selection
  deduplicatedCount: number; // How many were removed as duplicates
  totalTokens: number; // Total tokens for selected sources
  selectionStrategy: string; // Description of strategy used
  qualityMetrics: {
    averageRelevance: number;
    minRelevance: number;
    maxRelevance: number;
    highQualityCount: number; // Sources with relevance > 80
  };
}
...
export interface SourceSelectionConfig {
  baseLimit?: number; // Base source limit (default: 25)
  extendedLimit?: number; // Extended limit for high-quality sources (default: 30)
  highQualityThreshold?: number; // Threshold for extension (default: 70)
  tokenBudget?: number; // Maximum total tokens (default: 16800)
  minRelevanceScore?: number; // Minimum relevance to include (default: 40)
  semanticSimilarityThreshold?: number; // Cosine similarity for dedup (default: 0.85)
  enableSemanticDedup?: boolean; // Enable semantic deduplication (default: true)
}
 ...
 */
export async function selectSourcesForSynthesis(
  ...
  logger.info(
    `🎯 [SOURCE-SELECTOR] Starting selection: ${rankedSources.length} sources, ` +
    `base limit=${baseLimit}, extended limit=${extendedLimit}`
  );

  // ===== STEP 1: Filter by minimum relevance =====
  const qualifiedSources = rankedSources.filter(
    s => s.relevanceScore >= minRelevanceScore
  );

  if (qualifiedSources.length < rankedSources.length) {
    logger.info(
      `📊 [SOURCE-SELECTOR] Filtered out ${rankedSources.length - qualifiedSources.length} ` +
  ...
  const highQualitySources = qualifiedSources.filter(
    s => s.relevanceScore >= highQualityThreshold
  );
  ...
  if (highQualitySources.length > baseLimit) {
    ...
    logger.info(
      `📈 [SOURCE-SELECTOR] Extended limit to ${selectionLimit} ` +
  ...
  logger.info(
    `✂️ [SOURCE-SELECTOR] Selected top ${candidateSources.length} sources ` +
  ...
  if (enableSemanticDedup && candidateSources.length > 1) {
    const { unique, duplicateCount } = await deduplicateSemanticallySimilar(
      candidateSources,
    ...
    if (duplicateCount > 0) {
      logger.info(
        `🔍 [SOURCE-SELECTOR] Removed ${duplicateCount} semantically similar sources, ` +
  ...
  for (const rankedSource of candidateSources) {
    ...
    if (currentTokens + sourceTokens > tokenBudget) {
      logger.warn(
        `⚠️ [SOURCE-SELECTOR] Token budget reached: ${currentTokens} + ${sourceTokens} > ${tokenBudget}, ` +
  ...
  const result: SourceSelectionResult = {
    selectedSources,
    totalSources: rankedSources.length,
    selectedCount: selectedSources.length,
    deduplicatedCount,
    totalTokens: currentTokens,
    selectionStrategy: determineStrategyDescription(
  ...
  logger.info(
    `✅ [SOURCE-SELECTOR] Selection complete in ${duration}ms: ` +
  ...
  logger.info(`🏆 [SOURCE-SELECTOR] Top 5 selected sources:`);
  selectedSources.slice(0, 5).forEach((source, idx) => {
    const title = source.citation.substring(0, 60);
    logger.info(
      `  ${idx + 1}. [${source.relevanceScore}] ${source.sourceType}: ${title}... (${source.estimatedTokens} tokens)`
    );
  });
 ...
 */
async function deduplicateSemanticallySimilar(
  sources: RankedSource[],
  similarityThreshold: number
): Promise<{ unique: RankedSource[]; duplicateCount: number }> {
  const unique: RankedSource[] = [];
  ...
  for (const source of sources) {
    ...
    for (const existingSource of unique) {
      ...
      if (similarity > similarityThreshold) {
        isDuplicate = true;
        logger.debug(
          `🔍 [SOURCE-SELECTOR] Duplicate detected: similarity=${similarity.toFixed(2)}, ` +
 ...
 */
function extractTextForComparison(rankedSource: RankedSource): string {
 ...
 */
function formatSourceForSynthesis(rankedSource: RankedSource): SelectedSource {
  const source = rankedSource.source;
  const type = rankedSource.sourceType;
 ...
 */
function determineStrategyDescription(
  totalSources: number,
 ...
 */
export function formatSelectedSourcesForSynthesis(
  selectedSources: SelectedSource[]
): string {
