...
import { getRouterModel } from '../providers'; // Gemini 2.5 Flash Lite

/**
 * Query category types for diabetes research
 */
export type QueryCategory =
  | 'drug_safety'        // Medication side effects, interactions, safety
  | 'new_research'       // Latest research, clinical trials, new treatments
  | 'treatment'          // Treatment guidelines, therapy decisions
  | 'nutrition'          // Diet, food, recipes, nutrition science
  | 'general';           // General diabetes education, definitions
...
export interface QueryAnalysis {
  ...
  pubmedRatio: number;        // 0.0 - 1.0 (percentage of API sources from PubMed)
  medrxivRatio: number;       // 0.0 - 1.0 (percentage from medRxiv)
  clinicalTrialsRatio: number; // 0.0 - 1.0 (percentage from ClinicalTrials.gov)
  confidence: number;         // 0.0 - 1.0 (how confident the categorization is)
}
...
export async function analyzeQuery(
  ...
  try {
    ...
    const result = await ai.generate({
      model: getRouterModel(), // Gemini 2.5 Flash Lite - fast & cheap
      config: {
    ...
    console.log(
      `✅ [QUERY-ANALYZER] Categorized as "${analysis.category}" ` +
...
function getFallbackAnalysis(query: string): QueryAnalysis {
  ...
  if (
    /yan etki|etkileş|güvenli mi|side effect|interaction|contraindic|doz/i.test(query)
  ) {
    return {
      category: 'drug_safety',
      pubmedRatio: 0.7,
  ...
  if (
    /latest|yeni|güncel|202[4-6]|breakthrough|recent|clinical trial/i.test(query)
  ) {
    return {
      category: 'new_research',
      pubmedRatio: 0.5,
  ...
  // Nutrition keywords
  if (
    /beslenme|nutrition|diet|food|yemek|tarif|recipe|carb|protein/i.test(query)
  ) {
    return {
      category: 'nutrition',
      pubmedRatio: 0.8,
  ...
  // Treatment keywords
  if (
    /tedavi|treatment|therapy|guideline|protocol|hedef|target/i.test(query)
  ) {
    return {
      category: 'treatment',
      pubmedRatio: 0.65,
 ...
 */
export function calculateSourceCounts(
  ...
  if (diff !== 0) {
    ...
    const maxRatio = Math.max(
      analysis.pubmedRatio,
      analysis.medrxivRatio,
      analysis.clinicalTrialsRatio
    );
  ...
  console.log(
    `📊 [QUERY-ANALYZER] Source distribution: ` +
