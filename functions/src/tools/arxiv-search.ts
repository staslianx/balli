...
export interface ArxivPaperResult {
  arxivId: string; // e.g., "2301.12345v1"
  title: string;
  authors: string[]; // List of author names
  abstract: string;
  published: string; // Publication date (YYYY-MM-DD)
  updated: string; // Last update date
  categories: string[]; // arXiv categories (e.g., ["q-bio.QM", "cs.LG"])
  url: string; // Link to paper on arXiv
  pdfUrl: string; // Direct link to PDF
  journal: string; // Always "arXiv (preprint)"
  comments: string | null; // Comments if available
}
...
export async function searchArxiv(
  ...
  categories?: string[], // e.g., ['q-bio', 'cs']
  yearsBack?: number // Only search papers from last N years
 ...
 */
async function parseXMLResponse(xmlData: string): Promise<ArxivPaperResult[]> {
  return new Promise((resolve) => {
    parseString(xmlData, (err, result) => {
      ...
      const papers: ArxivPaperResult[] = entries.map((entry: any) => {
        // Extract arXiv ID from the full ID URL
        const fullId = entry.id?.[0] || '';
        const arxivId = fullId.split('/abs/')[1] || fullId;

        // Extract authors
        const authors = (entry.author || []).map((a: any) => a.name?.[0] || 'Unknown');
 ...
 */
export function formatArxivForAI(papers: ArxivPaperResult[]): string {
  if (papers.length === 0) {
