...
export interface StatisticalValidation {
  ...
  // Method 2: Mean Absolute Percentage Error
  mape: number;

  // Method 3: Root Mean Square Error
  rmse: number;

  // Method 4: Pearson Correlation Coefficient
  pearsonR: number;

  // Method 5: Lin's Concordance Correlation Coefficient
  linsCCC: number;

  // Method 6: Bland-Altman Analysis
  blandAltman: {
    meanDifference: number;     // Average bias
    standardDeviation: number;  // Variability
    lowerLOA: number;          // Lower limit of agreement
    upperLOA: number;          // Upper limit of agreement
    systematicBias: boolean;    // Is there systematic over/under-estimation?
  };
...
export interface ValidationThresholds {
  carbs: {
    maeMax: number;    // Max MAE for carbohydrates (g)
    mapeMax: number;   // Max MAPE for carbohydrates (%)
    pearsonMin: number; // Min Pearson r
  };
  fat: {
    mapeMax: number;   // Max MAPE for fat (%)
    pearsonMin: number; // Min Pearson r
  };
  protein: {
    mapeMax: number;   // Max MAPE for protein (%)
    pearsonMin: number; // Min Pearson r
  };
}
 ...
 */
export function calculateMAE(data: DataPoint[]): number {
 ...
 */
export function calculateRMSE(data: DataPoint[]): number {
  if (data.length === 0) return 0;
 ...
 */
export function calculatePearsonR(data: DataPoint[]): number {
 ...
 */
export function calculateLinsCCC(data: DataPoint[]): number {
  if (data.length < 2) return 0;
 ...
 */
export function calculateBlandAltman(data: DataPoint[]): {
 ...
 */
export function calculateErrorDistribution(data: DataPoint[]): {
  mean: number;
  median: number;
  min: number;
  max: number;
 ...
 */
export function performStatisticalValidation(
  data: DataPoint[],
  nutrientType: 'carbs' | 'fat' | 'protein',
  ...
  if (mape > nutrientThresholds.mapeMax) {
    failureReasons.push(
      `MAPE ${mape.toFixed(1)}% exceeds threshold ${nutrientThresholds.mapeMax}%`
    );
  }
  ...
  if (pearsonR < nutrientThresholds.pearsonMin) {
    failureReasons.push(
      `Pearson r ${pearsonR.toFixed(2)} below threshold ${nutrientThresholds.pearsonMin}`
    );
  }
  ...
  if (nutrientType === 'carbs' && 'maeMax' in nutrientThresholds && mae > nutrientThresholds.maeMax) {
    failureReasons.push(
      `MAE ${mae.toFixed(1)}g exceeds critical threshold ${nutrientThresholds.maeMax}g for carbs`
    );
  }
  ...
  if (blandAltman.systematicBias) {
    ...
    failureReasons.push(
      `Systematic bias detected: EDAMAM ${biasDirection} by ${Math.abs(blandAltman.meanDifference).toFixed(1)}g on average`
    );
  }
