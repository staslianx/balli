 ...
 */
function getFallbackQuestions(strategy: string): string[] {
 ...
 */
export function calculateConfidence(strategy: string, sourceCount: number): number {
  // Higher confidence when backed by sources
  if ((strategy === 'deep_research' || strategy === 'DEEP_RESEARCH') && sourceCount >= 3) return 95;
  if ((strategy === 'medical_sources' || strategy === 'WEB_SEARCH') && sourceCount >= 2) return 90;
  if (strategy === 'direct_knowledge' || strategy === 'MODEL') return 85; // Still high, just no external validation
  return 75; // Default
}
 ...
 */
export function mapCredibilityLevel(level: string): 'medical_source' | 'peer_reviewed' | 'clinical_trial' | 'expert' {
  switch (level) {
 ...
 */
export function formatSourcesWithTypes(
  exa: ExaSearchResult[],
  ...
  console.log(
    `📋 [RESEARCH-HELPERS] Formatted ${sources.length} sources: ` +
