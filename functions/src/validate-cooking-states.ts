...
interface TestResult {
  ...
  input: string;
  recognized: boolean;
  edamam_per_100g: NutritionValues | null;
 ...
 */
async function testWithEdamam(ingredientInput: string): Promise<NutritionValues | null> {
...
function determineCookingState(
  ...
  const distanceToRaw = Math.sqrt(
    Math.pow(edamam.carbs - raw.carbs, 2) +
