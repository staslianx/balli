...
import { batchSearchUSDA, USDANutrition } from './utils/usda-client';
import {
...
/**
 * Test ingredient with Turkish and English names
 */
interface TestIngredient {
  turkish: string;        // Turkish name for EDAMAM
  english: string;        // English name for USDA lookup
  category: 'carb' | 'protein' | 'fat' | 'vegetable' | 'other';
}

/**
 * EDAMAM response structure (simplified)
 */
interface EdamamResponse {
  calories: number;
  totalWeight: number;
  totalNutrients: {
    CHOCDF?: { quantity: number };  // Carbohydrates
...
/**
 * Individual ingredient test result
 */
interface IngredientTestResult {
  ...
  englishName: string;
  category: string;
  groundTruth: USDANutrition | null;
  edamam: USDANutrition | null;
  recognized: boolean;
  errors?: {
    calories: number;
    carbs: number;
...
interface ValidationReport {
  metadata: {
    ...
    usdaSuccessRate: number;
    edamamRecognitionRate: number;
  };
  ingredients: IngredientTestResult[];
  statisticalValidation: {
    carbs: StatisticalValidation;
    protein: StatisticalValidation;
    fat: StatisticalValidation;
  };
  decision: {
    recommendation: 'SHIP' | 'DO_NOT_SHIP' | 'CONDITIONAL_SHIP';
 ...
 */
const CORE_INGREDIENTS: TestIngredient[] = [
 ...
 */
const PROOF_OF_CONCEPT_INGREDIENTS: TestIngredient[] = [
  { turkish: '200g tavuk göğsü, ızgara', english: 'Chicken, broilers or fryers, breast, skinless, boneless, meat only, cooked, grilled', category: 'protein' },
  { turkish: '200g bulgur, pişmiş', english: 'Bulgur, cooked', category: 'carb' },
  { turkish: '2 yemek kaşığı zeytinyağı', english: 'Oil, olive, salad or cooking', category: 'fat' }
];
 ...
 */
async function testWithEdamam(turkishIngredient: string): Promise<USDANutrition | null> {
...
function calculateErrors(
  groundTruth: USDANutrition,
  edamam: USDANutrition
...
async function runValidation(
  ...
  const results: IngredientTestResult[] = [];

  // Step 1: Fetch ground truth from USDA
  console.log(`📊 Step 1: Fetching ground truth from USDA (${testIngredients.length} ingredients)`);
  const usdaSearches = new Map(
    testIngredients.map(ing => [ing.turkish, ing.english])
  );
  const usdaResults = await batchSearchUSDA(usdaSearches);
  ...
  for (const ingredient of testIngredients) {
    ...
    const result: IngredientTestResult = {
  ...
  const carbsData: DataPoint[] = validResults.map(r => ({
    groundTruth: r.groundTruth!.carbohydrates,
    edamam: r.edamam!.carbohydrates,
    ingredient: r.ingredient
  }));

  const proteinData: DataPoint[] = validResults.map(r => ({
    groundTruth: r.groundTruth!.protein,
    edamam: r.edamam!.protein,
    ingredient: r.ingredient
  }));

  const fatData: DataPoint[] = validResults.map(r => ({
  ...
  const statisticalValidation = {
    carbs: performStatisticalValidation(carbsData, 'carbs'),
    protein: performStatisticalValidation(proteinData, 'protein'),
    fat: performStatisticalValidation(fatData, 'fat')
  };

  // Step 4: Ship/Don't Ship Decision
  console.log(`\n🎯 Step 4: Ship/Don't Ship Decision`);

  const decision = makeShipDecision(
    results,
    statisticalValidation,
    testType
  );
...
async function main() {
  ...
  let testIngredients: TestIngredient[];
  let testType: string;
